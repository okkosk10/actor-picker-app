...
- 각 특집은 최소 2개, 최대 ${targetSizeGB ? Math.max(30, Math.ceil(targetSizeGB / 2)) : 30}개 videoId를 포함하세요. 단, 사용자가 '싹다', '모두', '전부'처럼 전체 포함을 요청한 경우에는 개수 제한보다 전체 포함을 우선하세요.
- 같은 videoId가 너무 많은 테마에 중복되지 않게 하세요.
- **배우 다양성**: 특정 배우 한 명을 주제로 하는 테마는 전체 테마 중 최대 1개로 제한하세요. 같은 배우가 여러 테마에 반복 등장하지 않도록 하세요.
- **태그 기반 요청 시 테마 내 배우 다양성**: [사용자 요청]에 배우 이름이 없고 태그/장르만 명시된 경우, 각 테마 안에 여러 배우의 영상이 골고루 포함되도록 하세요. 후보에 특정 배우가 많더라도 한 배우가 테마의 절반 이상을 차지하지 않도록 하세요.
...
- **중요**: '싹다', '모두', '전부' 등 전체 포함 요청이 있으면 조건에 맞는 영상을 최대한 하나의 큰 폴더에 모아 주세요. '배우당 N개씩' 요청은 배우마다 별도 폴더를 만들지 말고, 하나의 폴더 안에 각 배우에서 N개씩 골라 포함하세요.${
    targetSizeGB
      ? (() => {
          const wantsOne = /하나[의\s]*폴더|폴더[를을\s]*하나|한\s*개[의\s]*폴더|하나로\s*만|하나짜리|\d+\s*(?:기가|gb)짜리/i.test(customPrompt || '')
          if (wantsOne) {
